...
class MixedSparseGatedMLPFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if activation_forward == 'relu':
            xL = torch.relu(y1) 
        elif activation_forward == 'silu': # silu(x + \delta x) \approx silu(x) + \delta x * silu'(x) \approx silu(x) + \delta x * mask
            xL = y1 * torch.sigmoid(y1)
        elif activation_forward == 'gelu': # gelu(x + \delta x) \approx silu(x) + \delta x * gelu'(x) \approx gelu(x) + \delta x * mask
            xL = torch.gelu(y1)
        ...
        x3_save = torch.zeros((*x3.shape[:-1], actual_maintain_channel), dtype=x3.dtype, device=x3.device)
        xL_save = torch.zeros((*xL.shape[:-1], actual_maintain_channel), dtype=xL.dtype, device=xL.device)
        xR_save = torch.zeros((*xR.shape[:-1], actual_maintain_channel), dtype=xR.dtype, device=xR.device)
        ...
        ctx.save_for_backward(x1, y1_lora_a, mask, y1_main_mask, xL_save, xR_save, y2_lora_a, x3_save, y3_lora_a, w_gate, b_gate, w_gate_lora_a, w_gate_lora_b, w_up, b_up, w_up_lora_a, w_up_lora_b, w_down, b_down, w_down_lora_a, w_down_lora_b)
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        x1, y1_lora_a, mask, y1_main_mask, xL_save, xR_save, y2_lora_a, x3_save, y3_lora_a, w_gate, b_gate, w_gate_lora_a, w_gate_lora_b, w_up, b_up, w_up_lora_a, w_up_lora_b, w_down, b_down, w_down_lora_a, w_down_lora_b = ctx.saved_tensors
