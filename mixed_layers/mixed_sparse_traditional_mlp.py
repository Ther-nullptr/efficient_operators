...
class MixedSparseTraditionalMLPFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        zero_counts_per_channel = (x2 == 0).sum(
            dim=-2
        )  # [bs, seq_len, hidden_dim] -> [bs, hidden_dim]
        actual_maintain_channel = min(
            int(sparsity_ratio * x2.size(-1)), maintain_channels
        )
        # record the top sparsity_ratio channels
        _, topk_indices = zero_counts_per_channel.topk(
            actual_maintain_channel, dim=-1, largest=False
        )

        # delete the sparse channels, and also delete the corresponding x2 channels
        x2_save = torch.zeros(
            (*x2.shape[:-1], actual_maintain_channel), dtype=x2.dtype, device=x2.device
        )
